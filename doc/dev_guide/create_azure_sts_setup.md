...
If missing access please add the those accesses.
...
2. Creating OIDC configuration files inside the container
...
2. Create Openshift cluster up and running, update the system `kubeconfig` with cluster's kube config.
...
```
In that case you need to try with the below command,
...
8. You can verify that the configuration are public available by using your browser (Chrome, Firefox, etc.) and enter the URL of: 
```
