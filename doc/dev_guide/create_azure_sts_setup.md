...
These steps are common for Openshift and Minikube.
...
If access is missing, please ensure the necessary permissions are added.
...
**Where:**
- Cluster Issuer URL: {OPENID_CONTAINER_URL}( It mentioned in this coming sections)
- Namespace: Namespace where NooBaa is installed
- Service Account: ServiceAccount name, for
    - Operator: `noobaa`
    - Core: `noobaa-core`
...
NooBaa uses three distinct ServiceAccounts they are `noobaa`, `noobaa-core`, and `noobaa-endpoint` for operator, core, and endpoint components respectively,

When the serviceAccountToken is mounted for each NooBaa component, the tokens will have different subjects:
- Operator: system:serviceaccount:{namespace}:noobaa
- Core: system:serviceaccount:{namespace}:noobaa-core
- Endpoint: system:serviceaccount:{namespace}:noobaa-endpoint
...
### Create OIDC Container
1. Create the Azure Container (for hosting the OIDC configurations).
...
Please make it public access after creation.
...
This document provide two way of testing Azure STS, one is using openshift cluster and other one is using minikube

- [Azure STS Setup for OpenShift](#create-azure-sts-setup-on-openshift-cluster)

  The OpenShift Azure STS document section details the setup of Azure STS and the installation of NooBaa with STS configuration. It also provides steps for testing the backing store using STS credentials.

- [Azure STS Setup for Minikube](#create-azure-sts-setup-on-minikube-cluster)

  This document section details the minimum Azure STS setup for Minikube, specifically demonstrating the configuration with an Nginx pod. The primary goal is to verify that the Azure STS configuration is correct and that the configured OIDC provider can successfully validate the request.
...
The setup involves:
1. Create OIDC Azure blob Container
2. Create OIDC configuration files inside the container
...
1. Create OIDC container as mentioned in  [Create OIDC Container](#create-OIDC-container)
2. Create Openshift cluster up and running, update the system `kubeconfig` with cluster's kubeconfig.

3. Patch the cluster authentication config with openid container URl, setting `spec.serviceAccountIssuer`.
...
4. fetch the Service Account signing public key for Openshift cluster:
...
5. Create a file named `keys.json` that contains the following information:
...
Note: If your OpenSSL package is `LibreSSL` then you might get an error like this,

```
Invalid cipher '-outform'
```
In that case you need to try with the below commands,


- `<public_signing_key_id>` is generated from the public key with:
```bash
sed 's/BEGIN RSA PUBLIC KEY/BEGIN PUBLIC KEY/; s/END RSA PUBLIC KEY/END PUBLIC KEY/' sa-signer.pub > sa-signer-spki.pem
```


- `<public_signing_key_modulus>` is generated from the public key with:
```bash
openssl pkey -pubin -in sa-signer-spki.pem -text -noout \
...
```bash
- Step 1: openssl pkey -pubin -in sa-signer-spki.pem -text -noout \
| sed -n 's/Exponent: //p'

- Step 2: printf '%06x' 65537 | xxd -r -p | openssl base64
```

6. Create a file named `openid-configuration` that contains the following information:
...
**Where:**
- You need to paste the value of `${OPENID_CONTAINER_URL}`.

7. Upload the OIDC configuration:
...
8. You can verify that the configurations are public available by using your browser (Chrome, Firefox, etc.) and enter the URL of: 
```
...
10. Fetch the Projected service account token and save it in `WEB_FEDERATED_TOKEN`.

```bash
WEB_FEDERATED_TOKEN=$(kubectl exec nginx -- cat /var/run/secrets/tokens/oidc-token)
```

11. Fetch Access Token using Federated Identity Token(JWT Token) and use that access token to access the resource (In our case, it's the Storage Account and Container storage).
...
12. Test Azure STS with Noobaa Installation
...
13. Create backingstore using Azure STS credentials
...
#### Requirements:
- Azure account (with Admin permission)
- Azure CLI
- minikube
- openssl
...
3. After the minikube node has started, fetch the Service Account signing public key:
...
4. Create OIDC configuration and push those files to OIDC container, As mentioned here [Build an OIDC configuration](#build-an-oidc-configuration) step 5 to 8

### Federated Token Test:
We would create an nginx pod and fetch the ServiceAccount token from it and then validate and see that we can get the credentials.

5. Create a nginx pod
...
6. Fetch the Projected service account token and save it in `WEB_FEDERATED_TOKEN`.

```bash
WEB_FEDERATED_TOKEN=$(kubectl exec nginx -- cat /var/run/secrets/tokens/oidc-token)
```

7. Fetch Access Token using Federated Identity Token(JWT Token) and use that access token to access the resource (In our case, it's the Storage Account and Container storage).
