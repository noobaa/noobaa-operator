		...
		Describe("Azure Blob backingstore", func() {
			...
			Context("Empty secret name", func() {
				It("Should Deny when secret is empty and no Azure STS credentials", func() {
					...
					err = validations.ValidateBSEmptySecretName(*bs)
					Ω(err).Should(HaveOccurred())
					Expect(err.Error()).To(Equal("Failed creating the Backingstore, please provide secret name or Azure STS credentials (clientId and tenantId)"))
				})
				It("Should Allow when secret is empty but Azure STS credentials (clientId and tenantId) are set", func() {
				...
				It("Should Deny when only ClientId is set without TenantId", func() {
					clientID := "azure-client-id"
					bs.Spec = nbv1.BackingStoreSpec{
						Type: nbv1.StoreTypeAzureBlob,
						AzureBlob: &nbv1.AzureBlobSpec{
			...
			Context("Full Azure STS backingstore validation", func() {
				It("Should Allow Azure STS with clientId and tenantId only", func() {
					clientID := "azure-client-id"
					tenantID := "azure-tenant-id"
					bs.Spec = nbv1.BackingStoreSpec{
						Type: nbv1.StoreTypeAzureBlob,
