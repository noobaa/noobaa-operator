...
func NewBucketRequest(
	...
	if r.OB == nil {
		...
		if !exists {
			...
			p.recorder.Eventf(r.OBC, nil, "Warning", "MissingBucketClass", "Provision", msg)
			return nil, errors.New(msg)
		}
		if r.BucketClass.Status.Phase != nbv1.BucketClassPhaseReady {
			msg := fmt.Sprintf("BucketClass %q is not ready", bucketClassName)
			p.recorder.Eventf(r.OBC, nil, "Warning", "BucketClassNotReady", "Provision", msg)
			return nil, errors.New(msg)
		}
		...
		if util.IsRemoteObcAnnotation(r.OBC.Annotations) {
			...
			if externalErr != nil {
			p.recorder.Eventf(r.OBC, nil, "Warning", "RemoteOBCExternalDNSUnavailable", "Provision",
				"using internal DNS details: %v", externalErr)
			} else {
