...
func (r *Reconciler) completeReconcile(err error) (reconcile.Result, error) {
	...
	if err != nil {
		if perr, isPERR := err.(*util.PersistentError); isPERR {
			...
			if r.Recorder != nil {
				r.Recorder.Eventf(r.NamespaceStore, nil, corev1.EventTypeWarning, perr.Reason, "Reconcile", perr.Message)
			}
		} else {
		...
		if exist && phaseInfo.Phase != r.NamespaceStore.Status.Phase {
			...
			if r.Recorder != nil {
				r.Recorder.Eventf(r.NamespaceStore, nil, phaseInfo.Severity, phaseName, "Reconcile", desc)
			}
		} else {
			r.SetPhase(
