...
func (r *Reconciler) Reconcile() (reconcile.Result, error) {
	...
	if err != nil {
		if perr, isPERR := err.(*util.PersistentError); isPERR {
			...
			if r.Recorder != nil {
				r.Recorder.Eventf(r.BucketClass, nil, corev1.EventTypeWarning, perr.Reason, "Reconcile", perr.Message)
			}
		} else {
			res.RequeueAfter = 3 * time.Second
		...
		if r.BucketClass.Status.Mode != "OPTIMAL" && r.BucketClass.Status.Mode != "" {
			if r.Recorder != nil {
				r.Recorder.Eventf(r.BucketClass, nil, corev1.EventTypeWarning, r.BucketClass.Status.Mode, "Reconcile", r.BucketClass.Status.Mode)
			}
		}
		r.SetPhase(
