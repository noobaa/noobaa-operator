...
func (r *Reconciler) Reconcile() (reconcile.Result, error) {
	...
	if err != nil {
		if perr, isPERR := err.(*util.PersistentError); isPERR {
			...
			if r.Recorder != nil {
				r.Recorder.Eventf(r.BucketClass, nil, corev1.EventTypeWarning, perr.Reason, perr.Reason, perr.Message)
			}
		} else {
			res.RequeueAfter = 3 * time.Second
		...
		if r.BucketClass.Status.Mode != "OPTIMAL" && r.BucketClass.Status.Mode != "" {
			if r.Recorder != nil {
				r.Recorder.Eventf(r.BucketClass, nil, corev1.EventTypeWarning, r.BucketClass.Status.Mode, r.BucketClass.Status.Mode, r.BucketClass.Status.Mode)
			}
		}
		r.SetPhase(
...
func (r *Reconciler) updateArchivePolicyForExistingBuckets(bucketNames []string) error {
	...
	for _, bucketName := range bucketNames {
		...
		if err != nil {
			return fmt.Errorf("failed to read bucket %q for archive policy update: %w", bucketName, err)
		}
		...
		if err != nil {
			return fmt.Errorf("failed to apply archive policy to bucket %q: %w", bucketName, err)
		}
