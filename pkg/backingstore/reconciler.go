...
func (r *Reconciler) completeReconcile(err error) (reconcile.Result, error) {
	...
	if err != nil {
		if perr, isPERR := err.(*util.PersistentError); isPERR {
			...
			if r.Recorder != nil {
				r.Recorder.Eventf(r.BackingStore, nil, corev1.EventTypeWarning, perr.Reason, "Reconcile", perr.Message)
			}
		} else {
		...
		if exist {
			if phaseInfo.Phase != r.BackingStore.Status.Phase {
				...
				if r.Recorder != nil {
					r.Recorder.Eventf(r.BackingStore, nil, phaseInfo.Severity, phaseName, "Reconcile", desc)
				}
			}
		} else {
